...
if (isset($_POST['find'])) {
  ...
  include 'template/header.php'; ?>
    <link rel="stylesheet" type="text/css" href="bootstrap/bootstrap-5.2.2-dist/css/bootstrap.min.css">
  <body>
  ...
  <div class="ftco-section-reservation"   >
    ...
    <section class="ftco-section bg-light">
      <div class="container">
        ...
        <center>
        <div>
        <form class="d-flex mx-auto" role="search" method="post">
            <input class="form-control me-2 col-md-3" id="search" type="search" id="form" placeholder="search by product or category"
                        aria-label="Search">
            <button class="btn btn-outline-success" type="submit" id="submit">Search</button>
            <?php
  if(isset($_POST['submit'])){
    $search = $_POST['search'];
    $query = "SELECT * FROM restaurent_info WHERE restaurent_name LIKE '%$search%'";
    $result = mysqli_query($conn, $query);
    if(mysqli_num_rows($result) > 0){
      while($row = mysqli_fetch_assoc($result)){
        echo"record found";
        // echo "ID: " . $row['id'] . " Name: " . $row['restaurent_name'] . "<br>";
      }
    } else {
...
<!-- <form action="" method="post">
  ...
  <script type="text/javascript" src="bootstrap/bootstrap-5.2.2-dist/js/bootstrap.min.js"></script>
</html>
<?php 
  }else{
